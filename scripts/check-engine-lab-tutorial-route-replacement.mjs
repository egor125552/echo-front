...
try {
 ...
 navigation.selectTarget(playerId,'warehouse',now+16);
 game.api.handleInput(playerId,{navigationTogglePressed:true},now+17);
 assert.equal(navigation.stateFor(playerId).target?.id,'warehouse');
 assert.equal(phase(),'select-second-crate','wrong replacement must return to selecting the needed loot');
 assert.equal(tutorial.describe(playerId).rifleCollected,true);
 assert.equal(tutorial.describe(playerId).neededLoot,'armor');
 navigation.selectTarget(playerId,'crate:crate-upper-armor',now+18);
 game.api.handleInput(playerId,{navigationTogglePressed:true},now+19);
 assert.equal(phase(),'follow-second-crate','correct replacement must resume second-crate navigation');
 navigation.selectTarget(playerId,'crate:two-storey-ground-armor',now+20);
 game.api.handleInput(playerId,{navigationTogglePressed:true},now+21);
 assert.equal(phase(),'follow-second-crate','replacing an armor route with another armor route must remain valid');
 console.log('ENGINE_LAB_TUTORIAL_ROUTE_REPLACEMENT_OK');
 lab.finish();
