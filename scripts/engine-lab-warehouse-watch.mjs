...
const room='warehouse-watch-'+crypto.randomUUID().slice(0,12);
const watchWindowMs=Math.max(60_000,Math.min(12*60_000,Number(process.env.ECHO_WAREHOUSE_WATCH_MINUTES??12)*60_000));
...
for(let turn=0;;turn++){
 ...
 if(state.simulatedMs>=watchWindowMs){record('warehouse-observation-window-ended','Observation window reached without ending game');break;}
 if(turn%20===0&&turn>0){
 ...
 if(phase==='vehicle'&&player.y<1.6&&Math.hypot(player.x-60,player.z)<36){
   phase='warehouse-watch';record('warehouse-watch-start','Remain near warehouse while real bot vehicles arrive');
 }
 ...
 } else if(phase==='warehouse-watch'){
   const enemy=state.visibleEntities?.find(e=>e.bot&&e.alive&&e.distanceMeters<32);
   if(enemy){
     input={forward:-.25,strafe:.65,fireHeld:true,sprint:false};
     action='warehouse-defend';details='Defend from actual bot '+enemy.id+' '+enemy.distanceMeters.toFixed(1)+'m';step=20;
   }else{
     const waypoint={x:80,z:player.z>7?0:player.z<-7?0:0};
     const nav=direction(player,waypoint);
     input=nav.distance>8?{forward:nav.forward,strafe:nav.strafe,sprint:false}:{};
     action='warehouse-wait';details='Stay near warehouse door, watch actual bot traffic';step=20;
   }
 ...
 if(turn%5===0)await observeWarehouse();
...
if(state.self?.alive===false){
  ...
  for(let spectatorTurn=0;;spectatorTurn++){
    ...
    if(spectatorTurn%2===0)await observeWarehouse();
    if(advanced.simulatedMs>=watchWindowMs){
      console.log('WAREHOUSE_OBSERVATION_WINDOW_DONE',advanced.gameTime);
      break;
    }
