...
try {
 ...
 // Reach the selected second crate, but change destination instead of opening it.
 events.emit('navigation:reached',{entityId:playerId,targetId:'crate:crate-ground-armor',targetKind:'crate',targetLoot:'armor',now:now+16});
 assert.equal(phase(),'interact-second-crate');
 navigation.selectTarget(playerId,'crate:crate-upper-armor',now+17);
 game.api.handleInput(playerId,{navigationTogglePressed:true},now+18);
 assert.equal(phase(),'follow-second-crate','switching away from an interaction must follow the replacement crate');
 assert.equal(tutorial.describe(playerId).neededLoot,'armor');
 ...
 assert.equal(phase(),'interact-second-crate');
 navigation.selectTarget(playerId,'warehouse',now+20);
 game.api.handleInput(playerId,{navigationTogglePressed:true},now+21);
 assert.equal(phase(),'select-second-crate','unrelated destination must not leave the tutorial demanding an old crate');
 assert.equal(tutorial.describe(playerId).rifleCollected,true);
 console.log('ENGINE_LAB_TUTORIAL_INTERACTION_REPLACEMENT_OK');
 lab.finish();
