...
export async function setup(ctx) {
  ...
  ctx.events.on("network:welcome", ({ team: joinedTeam }) => {
    team = joinedTeam;
    announce(
      `Вы в команде ${team}. Первый раунд начинаете с пистолетом: пятьдесят патронов в магазине и триста пятьдесят в запасе, всего четыреста. ` +
      "Чтобы прицелиться, поворачивайтесь Q и E: когда звук противника окажется по центру, он прямо перед вами и можно стрелять X. " +
      "Стрелки влево и вправо — это только короткие боковые шаги, они не поворачивают. Если вы сами шагнули вправо, неподвижный противник естественно сместится влево относительно головы, и наоборот. " +
      "После первого раунда откроется автомат. У прицела есть небольшой допуск. Shift — бег. Удерживайте Z и нажимайте влево или вправо, чтобы сменить открытое оружие.",
      { interrupt: true },
    );
  });
