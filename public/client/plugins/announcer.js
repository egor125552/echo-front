...
export async function setup(ctx) {
  ...
  ctx.events.on("network:welcome", ({ team: joinedTeam }) => {
    ...
    announce(
      ...
      "Можно одновременно идти вперёд и целиться. Стрелки вверх и вниз двигают вас, а стрелки влево и вправо плавно поворачивают. Возле противника поворот сам замедляется, поэтому ловить идеальный центр не нужно: доверните звук примерно вперёд и стреляйте X. " +
      "Q и E — короткие боковые шаги. В пистолете сто патронов, ещё четыреста в запасе, всего пятьсот. R — перезарядка из запаса. После первого раунда откроется автомат и сложность станет обычной. Shift — бег. Удерживайте Z и нажимайте стрелки влево или вправо, чтобы сменить открытое оружие.",
      { interrupt: true },
    );
  });
