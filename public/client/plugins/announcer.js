...
export async function setup(ctx) {
  ...
  ctx.events.on("network:welcome", ({ team: joinedTeam }) => {
    team = joinedTeam;
    announce(
      `Вы в команде ${team}. Первый раунд учебный и заметно легче: противники в основном выходят перед вами, идут почти прямо на вас, медленнее открывают огонь, а прицел помогает сильнее. ` +
      "Можно одновременно идти вперёд и целиться. Стрелки вверх и вниз двигают вас, а стрелки влево и вправо плавно поворачивают. Возле противника поворот сам замедляется. Идеальный центр не нужен: достаточно довернуть звук примерно вперёд, рабочий сектор прицела широкий. X можно просто удерживать, пистолет сам будет стрелять в допустимом темпе. " +
      "Q и E — короткие боковые шаги. В пистолете сто патронов, ещё четыреста в запасе, всего пятьсот. R — перезарядка из запаса. После первого раунда откроется автомат. Shift — бег. Удерживайте Z и нажимайте стрелки влево или вправо, чтобы сменить открытое оружие.",
      { interrupt: true },
    );
  });
