...
export async function setup(ctx) {
  ...
  ctx.events.on("network:welcome", ({ team: joinedTeam }) => {
    team = joinedTeam;
    announce(
      `Вы в команде ${team}. Первый раунд учебный: противники в основном выходят перед вами и медленнее открывают огонь. ` +
      "Теперь точный центр ловить не нужно. Стрелки вверх и вниз двигают вас, стрелки влево и вправо поворачивают для ориентации по звуку, Q и E делают боковые шаги. При выстреле игра сама выбирает доступного противника в радиусе оружия, немного предпочитая тех, кто находится впереди и ближе. X можно просто удерживать. " +
      "В пистолете сто патронов, ещё четыреста в запасе, всего пятьсот. R — перезарядка из запаса. После первого раунда откроется автомат. Shift — бег. Удерживайте Z и нажимайте стрелки влево или вправо, чтобы сменить открытое оружие.",
      { interrupt: true },
    );
  });
