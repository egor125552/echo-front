...
export async function setup(ctx) {
  ...
  ctx.events.on("network:welcome", ({ team: joinedTeam }) => {
    team = joinedTeam;
    announce(
      `Вы в команде ${team}. Первый раунд учебный: противники меньше обходят сбоку, позже стреляют, а прицел заметно шире. ` +
      "Стрелки вверх и вниз — движение. Стрелки влево и вправо поворачивают вас: доведите звук противника примерно к центру и стреляйте X. " +
      "Q и E — короткие боковые шаги. В пистолете пятьдесят патронов и ещё триста пятьдесят в запасе. После первого раунда откроется автомат. Shift — бег. Z со стрелками влево или вправо — смена оружия.",
      { interrupt: true },
    );
  });
  ...
  ctx.events.on("game:event", (packet) => {
    ...
    if (packet.event === "match:started") {
      ...
      announce(round === 1 ? "Учебный раунд. В бой" : `Раунд ${round || ""}. В бой`, { interrupt: true });
    }
  });
}
