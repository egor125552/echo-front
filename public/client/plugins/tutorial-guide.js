...
const TEAM_MESSAGES = {
  ...
  strafe: "Хорошо. Теперь пройди боком примерно три метра влево или вправо. На клавиатуре удерживай стрелку влево или вправо. На сенсорном экране используй кнопку Влево или Вправо.",
...
const BATTLE_ROYALE_MESSAGES = {
  ...
  "select-rifle": "Теперь у тебя есть пистолет и автомат. Переключись на автомат. На клавиатуре удерживай Z и нажми стрелку вправо или влево. На сенсорном экране нажимай Следующее оружие или Предыдущее оружие, пока не услышишь Автомат.",
...
export async function setup(ctx) {
  ...
  function presentPhase(tutorial) {
    ...
    if (tutorial.mode === "battle-royale") {
      ...
      if (phase === "automatic-parachute") {
        present(BATTLE_ROYALE_MESSAGES[phase], {
          afterSpeech: () => network.send("tutorial:speech-complete", { phase }),
        });
      } else {
        present(BATTLE_ROYALE_MESSAGES[phase]);
      }
